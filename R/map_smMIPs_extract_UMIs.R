...
option_list = list(
  make_option(c("-b", "--bam.file"), type="character", default=NULL,help="Path to bam file [MENDATORY]", metavar="character"),
  ...
  make_option(c("-o", "--output"), type="character", default=NULL,help="Path for output files. A new folder which is named based on the -s parameter will be generated by default. If not supplied, the sample folder will be generated within the folder that contain the bam files", metavar="character"),
  make_option(c("-c", "--code"), type="character", default=getwd(),help="Path to smMIP tools source functions, smMIPs_Function.R file. If not supplied, it assume the code share the same folder as this code folder with this code (map_smMIPs_extract_UMIs.R)", metavar="character"),
  ...
  make_option(c("-O", "--OVERLAP"), type="numeric", default=0.95,help="Fine-tuning the overlap between reads and smMIPs. Used in the map.smip_to_site function.", metavar="character"),
  make_option(c("-M", "--MAPQ"), type="numeric", default=50,help="MAPQ cut-off. Used in the filter.on.mappingscore function", metavar="character"))
