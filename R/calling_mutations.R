...
option_list = list(
  ...
  make_option(c("-c", "--code"), type="character", default=getwd(),help="Path to smMIP tools source functions, smMIPs_Function.R file. If not supplied, it assume that smMIPs_Function.R share the same folder with this code (calling_mutations.R)", metavar="character"),
...
cat("Allele frequencies are now calulated based on both the plus and minus replicated DNA strands\n")
...
cat("Allele frequencies are now calulated based on overlapping smMIPs\n")
...
if(length(which(!is.na(data$samples$replicate)))>0){
  ...
  cat("Allele frequencies are now calulated based on sample replicates\n")
}
